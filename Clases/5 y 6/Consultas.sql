...
SELECT nombre AS nombre_producto, precio AS "USD", (precio * 1.09) AS "EUR"
FROM producto;
...
SELECT nombre AS Nombre_Fabricante , UPPER(SUBSTRING(nombre,1,2)) AS Nombre_Mayuscula FROM fabricante;
...
SELECT * FROM producto
WHERE (precio * 1.09) >= 80 AND (precio * 1.09) <= 300;
...
WHERE (producto.precio * 1.09) > 200 AND fabricante.id = 6
...
SELECT nombre, (precio * 100) AS "centecimos" FROM producto;
...
SELECT nombre AS "Nombre_Producto", precio * 1.09 AS "EUR"
FROM producto
